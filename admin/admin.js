...
function readOrder() {
    ...
    order.on("child_added", function (data) {
        ...
        <div class="card mb-3">
            <div class="card-body">
                <h5 class="card-title"> ORDEN: ${orderValue.order} </h5>
                <p class="card-text"> Total: ${orderValue.total} </p>

                <button type ="submit" style="color: white" class="btn btn-warning" onclick="updateOrder(${orderValue.id},'${orderValue.order}','${orderValue.total}')"><i class="far fa-edit"></i> Editar Orden </button>

                <button type ="submit" style="color: white" class="btn btn-danger" onclick="deleteOrder(${orderValue.id})"><i class="far fa-trash-alt"></i> Borrar Orden </button>
...
function reset() {
    ...
    <form class="border p-4 mb-4" id="form">
        <div class="form-group">
            <label> ORDEN</label>
            <input type="text" class="form-control" id="order" placeholder="Agregar Orden">
        </div>
        <div class="form-group">
            <label> TOTAL</label>
            <input type="text" class="form-control" id="total" placeholder="Agregar Total">
        </div>

            <button type="submit" id="button1" class="btn btn-primary">
            <i class="fas fa-plus"></i> Agregar Orden </button>
            <button style="display:none" id="button2" class="btn btn-success">
                           Actualizar Orden </button>
            <button style="display:none" id="button2" class="btn btn-danger">
                           Cancelar</button>
    </form>
